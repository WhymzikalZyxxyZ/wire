...
**Partition key, resolved during implementation.** This ADR was written assuming one event = one entry, keyed by that entry's `accountId`. The implementation resolved `docs/architecture/overview.md`'s open correlation question by making one event carry a whole transaction's entries at once (avoiding a stateful correlator — see that document's "Resolved design decision"), which means a single event can touch more than one account. The partition key actually used is the transaction's first entry's `accountId`. This still gives every entry *within one transaction* guaranteed ordering relative to each other (they're in the same message), but it does **not** give an account true global ordering across every transaction it's ever part of — two transactions touching the same account can land on different partitions if that account isn't each transaction's first entry. Named explicitly as an accepted limitation in `docs/RISKS.md`, not a silently narrower guarantee than advertised.

**Partition count, caught by a post-implementation code survey.** The partition key above is only meaningful if the topic actually has more than one partition — a code review found that `wire.transactions.raw` was being created with the broker's default partition count (typically 1), with consumer concurrency also left at its default of 1. Neither the code nor the tests were exercising real multi-partition behavior at all; the key existed but had nothing to partition across. Fixed by declaring explicit `NewTopic` beans and setting listener concurrency to match (see `KafkaConfig`) — see `docs/RISKS.md` for the full account of the gap and the fix.
