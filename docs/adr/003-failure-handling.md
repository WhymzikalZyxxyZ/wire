...
**Settled during implementation:** `wire.retry.max-attempts` (default 3) counts *retries after the initial attempt*, so a persistently-failing event sees 1 + 3 = 4 total calls to LEDGER before landing on the DLQ, with exponential backoff (200ms, 400ms, 800ms by default) between them. Retries are implemented as a small blocking loop inside the consumer's message-handling method (`Thread.sleep` between attempts), not Spring Kafka's non-blocking retry-topic feature (`@RetryableTopic`) — the simpler approach keeps the whole retry/DLQ contract in application code, directly unit-testable and easy to reason about, at the cost of that consumer thread blocking for the backoff duration rather than being free to service other partitions in the meantime. Non-blocking retry topics remain a real upgrade path if consumer throughput under a partial outage ever became the bottleneck being optimized for — not needed to prove the correctness claims this ADR makes. Tracked in `docs/RISKS.md`.
